...
def createARPPacket(OPER, mac_src, ip_src, mac_dst, ip_dst):
    ...
    arppacket = struct.pack("!HHBBHBBBBBB4sBBBBBB4s", HTYPE, PTYPE, HLEN, PLEN, OPER, mac_src_array[0],
                            mac_src_array[1], mac_src_array[2], mac_src_array[3], mac_src_array[4], mac_src_array[5],
                            ip_src_num, mac_dst_array[0], mac_dst_array[1], mac_dst_array[2], mac_dst_array[3],
                            mac_dst_array[4], mac_dst_array[5], ip_dst_num)
    return arppacket
...
def createSwitchRegistrationReplyPacket(switchvid):
    ...
    registrationreply = struct.pack("!HHBBHBBBBBBBBBBBB", HTYPE, PTYPE, HLEN, PLEN, SWITCH_REGISTER_REPLY,
                                    ...
                                    srcvid_array[5], srcdst_array[0], srcdst_array[1], srcdst_array[2], srcdst_array[3],
                                    srcdst_array[4], srcdst_array[5])
...
def createEchoReplyPacket(srcvid, dstvid):
    srcvid_array = getMacArray(srcvid)
    srcdst_array = getMacArray(dstvid)
    echoreply = struct.pack("!HHBBHBBBBBBBBBBBB", HTYPE, PTYPE, HLEN, PLEN, ECHO_REPLY, srcvid_array[0],
                            srcvid_array[1], srcvid_array[2], srcvid_array[3], srcvid_array[4], srcvid_array[5],
                            srcdst_array[0], srcdst_array[1], srcdst_array[2], srcdst_array[3], srcdst_array[4],
                            srcdst_array[5])
    return echoreply
...
def createSwitchRegistrationReplyPacket1(srcvid, dstvid):
    srcvid_array = getMacArray(srcvid)
    srcdst_array = getMacArray(dstvid)
    reply = struct.pack("!HHBBHBBBBBBBBBBBB", HTYPE, PTYPE, HLEN, PLEN, SWITCH_REGISTER_REPLY, srcvid_array[0],
                        srcvid_array[1], srcvid_array[2], srcvid_array[3], srcvid_array[4], srcvid_array[5],
                        srcdst_array[0], srcdst_array[1], srcdst_array[2], srcdst_array[3], srcdst_array[4],
                        srcdst_array[5])
    return reply
...
def createStorePacket(ip_to_store, vid_to_store, src_vid, dst_vid):
    ...
    store_packet = store_packet + struct.pack("!BBBBBB", src_vid_array[0], src_vid_array[1], src_vid_array[2],
                                              src_vid_array[3], src_vid_array[4], src_vid_array[5])
    #print 'StorePACKET = ',store_packet.encode('hex')
    # Put the destination vid now
    store_packet = store_packet + struct.pack("!BBBBBB", dst_vid_array[0], dst_vid_array[1], dst_vid_array[2],
...
# creates a packet of type RDV_DATA

def createRDV_DATA(myvid, dst, fwd, ttl, str):
    packet = struct.pack("!HHBBH", HTYPE, PTYPE, HLEN, PLEN, RDV_DATA)
    svid = struct.pack("!I", int(myvid, 2))
    dvid = struct.pack("!I", int(dst, 2))
    fvid = struct.pack("!I", int(fwd, 2))
    ttl = struct.pack("!B", ttl)
    z = struct.pack("!c", str)
    return (packet + svid + dvid + fvid + ttl + z)
...
# create a RDV_REPLY Packet
# GWS IS A LIST OF INTS HERE! AND REST ARE BINARY STRINGS

def createRDV_REPLY(gws, bucket_dist, myvid, dst):
    # First prepare header
...
# update the ForwardingDirective on the packet
def updateForwardingDirective(packet, fwd):
    newfwd = struct.pack("!I", int(fwd, 2))
    return packet[:16] + newfwd + packet[20:]

# update the TTL on the packet
def updateTTL(packet, ttl):
    newTTL = struct.pack("!B", int(ttl))
    return packet[:20] + newTTL + packet[21:]
...
# returns the forwarding directive in the string format
def getFwd(packet, L):
...
def printPacket(packet, L):
    ...
    if opcode not in OPERATION_NAMES:
        ...
        return
    print 'Type: ', OPERATION_NAMES[opcode], 'Source: ', bin2str(svid, L), 'Destination: ', bin2str(dvid,
                                                                                                    L), 'Payload: ', bin2str(
        payload, L)
